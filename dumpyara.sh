...
ORG=redmi-mt6765-dev #your GitHub org name
...
if [[ -d "${PWD}/external/Firmware_extractor" ]]; then
    ...
    git clone -q --recurse-submodules https://github.com/redmi-mt6765-dev/Firmware_extractor "${PWD}"/external/Firmware_extractor
fi
...
for image in boot vendor_boot vendor_kernel_boot; do
    if [[ -f "${image}.img" ]]; then
        LOGI "Extracting '${image}.img'..."

        # Prepare dirs
        mkdir -p "${image}/dtb" "${image}/dts" "${image}/ramdisk"

        # Unpack with unpackbootimg
        if ! ${UNPACKBOOTIMG} -i "${image}.img" -o "${image}/" > /dev/null 2>&1; then
            LOGE "unpackbootimg failed for ${image}.img"
            continue
        fi

        # Locate ramdisk
        RAMDISK_SRC=$(find "${image}" -name "${image}.img-*ramdisk" | head -n1)
        if [[ -z "${RAMDISK_SRC}" ]]; then
            LOGE "No ramdisk found in ${image}.img"
            continue
        fi

        LOGI "Detecting ramdisk format: '${RAMDISK_SRC}'"
        TYPE=$(file -b "${RAMDISK_SRC}")
        TMPFILE="${image}/ramdisk.cpio"

        case "$TYPE" in
            *LZ4*)
                LOGI "Detected LZ4"
                unlz4 "${RAMDISK_SRC}" "${TMPFILE}" || {
                    LOGE "unlz4 failed"
                    continue
                }
                ;;
            ...
            *XZ*)
                ...
                ;;
            *)
                LOGW "Unknown format, trying 7z..."
                if 7z e "${RAMDISK_SRC}" -o"${image}" -y > /dev/null 2>&1; then
                    EXTRACTED=$(find "${image}" -maxdepth 1 -type f ! -name '*.img*' | head -n1)
                    if [[ -n "$EXTRACTED" ]]; then
                        mv "$EXTRACTED" "${TMPFILE}"
                        LOGI "7z extraction successful"
                    else
                        LOGE "7z ran but no file extracted"
                        continue
                    fi
                else
                    LOGE "7z failed. Unsupported format."
                    continue
                fi
                ;;
        esac
        ...
        if (cd "${image}/ramdisk" && cpio -idm < ../ramdisk.cpio > /dev/null 2>&1); then
            LOGI "Ramdisk extracted successfully"
        else
            LOGE "Failed to extract CPIO from ${image}.img"
            continue
        fi

        # Clean up ramdisk intermediates
        rm -f "${TMPFILE}" "${RAMDISK_SRC}"

        # Extract DTBs using `uvx extract-dtb`
        LOGI "Extracting device trees from ${image}.img..."
        if uvx extract-dtb "${image}.img" -o "${image}/dtb" > /dev/null 2>&1; then
            rm -f "${image}/dtb/00_kernel"

            # Decompile DTBs to DTS
            for dtb in "${image}/dtb"/*.dtb; do
                [[ -f "$dtb" ]] || continue
                dtc -q -I dtb -O dts "$dtb" -o "${image}/dts/$(basename "$dtb" .dtb).dts" || \
                    LOGE "Failed to decompile $(basename "$dtb")"
            done
        else
            LOGE "uvx extract-dtb failed for ${image}.img"
        fi

        # Clean up empty DTB/DTS dirs
...
if ! uvx aospdtgen . --output "${WORKING}/aosp-device-tree" >> /dev/null 2>&1; then
    LOGE "Failed to generate AOSP device tree"
    rm -rf "${WORKING}/aosp-device-tree"
fi
...
if [[ -n $GIT_OAUTH_TOKEN ]]; then
    ...
    if [[ -z "$(git config --get user.email)" ]]; then
        git config user.email redmi-mt6765-dev@github.com
    fi
    if [[ -z "$(git config --get user.name)" ]]; then
        git config user.name redmi-mt6765-dev
    fi
